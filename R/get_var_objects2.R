########################################################
### Modified get_var_objects fun from svars package ####
...
get_var_objects <- function(x){
  ...
  if(inherits(x, "stvar")){
    assign("R", 2, envir = parent.frame())
  } else {
    assign("R", 1, envir = parent.frame())
  }
  ...
  if(inherits(x, "var.boot")){
    ...
    ### new stvar class
  }else   if(inherits(x, "stvar")){
    assign("u", x$u, envir = parent.frame())
    assign("Tob", x$Tobs, envir = parent.frame())
    assign("k", x$K, envir = parent.frame())
    assign("residY", x$u, envir = parent.frame())
    ###
  } else {
  ...
  if(inherits(x, "var.boot")){
    ...
    assign("p", x$nlags, envir = parent.frame())
    assign("y", t(x$Y), envir = parent.frame())
    assign("yOut", x$Y, envir = parent.frame())
    assign("type", x$det_term, envir = parent.frame())
    assign("coef_x", t(x$A_hat), envir = parent.frame())
    ###
